...
The audited Node 24.13.0 run in a clean dedicated worktree contained 626 tests: 625 passed, 1 was skipped by a platform condition, and none failed. The loaded source baseline was 86.21% lines, 79.40% branches, and 81.29% functions. CI runs the same coverage command on Node 24, so compare trends within the same Node and operating-system environment rather than treating small cross-environment changes as regressions.
