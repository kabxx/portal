...
本地目录保持原位引用。直接 `SKILL.md` URL、GitHub 地址、支持的压缩包和 Hub registry 中的 package 会下载到 `data/skills/`。`$<name> [task]` 前缀会为当前 turn 手动选择一个已启用 Skill。Manifest 校验、来源类型、手动选择、runtime 快照、存储和信任边界请参阅 [Skills 文档](skills.md)。
