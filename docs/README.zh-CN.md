...
[English](../README.md) | [API](api.md) | [Providers](providers.md) | [架构](architecture.md) | [安全说明](security.md) | [Skills](skills.md) | [MCP](mcp.md) | [贡献指南](contributing.md)
...
| `/thread close <thread-id>`                   | 关闭对应页面、runtime 和 MCP 连接       |
...
`run_command` 执行期间会在临时气泡中实时显示少量 stdout/stderr 尾部；完成后气泡会被简短结果替换。完整但受大小限制的结构化结果仍会返回网页模型。
