...
- **八个网页 Provider。** ChatGPT、Gemini、DeepSeek、豆包、Grok、GLM、Qwen 和 Kimi 使用同一套本地 thread 模型。
...
| Qwen     | 支持     | 支持        | 支持          | 支持     | 动态页面 Action          |
| Kimi     | 支持     | 支持        | 支持          | 支持     | 联网搜索                 |
...
专用浏览器与 portal 共享同一生命周期。关闭浏览器进程、浏览器崩溃或 CDP 连接断开时，portal 会执行受控关闭并停止活动 threads、jobs、API 和 MCP 服务。只关闭一个 Provider 标签页不会退出 portal；portal 会取消活动操作，并且只关闭与该页面绑定的 thread。如果它是 active thread，TUI 会返回首页。
...
输入 slash command 时，输入框下方还会显示上下文提示。提示会随输入筛选命令形式、显示关键参数，并且最多显示五条命令。`Up` 和 `Down` 可首尾循环浏览匹配命令，`Tab` 补全当前选中命令；`Enter` 始终提交输入框中的当前内容。
存在活动线程时，输入 `$` 或 `$前缀` 会用相同样式显示最多五个已启用 Skill 及其说明。`Up` 和 `Down` 可首尾循环浏览匹配 Skill，`Tab` 补全当前选中 Skill；`Enter` 保持默认提交行为。
...
| `Up` / `Down`                                             | 浏览命令或 Skill 提示、输入边界或输入历史     |
| `Tab`                                                     | 补全选中的命令提示、Provider 或 `$skill` 前缀 |
| `Ctrl+W`                                                  | 删除上一个单词                                |
| `Ctrl+U` 或 `Esc`                                         | 清空当前输入                                  |
